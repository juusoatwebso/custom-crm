#!/usr/bin/env python3
"""
Webso CRM Mini Setup — for testing only
========================================
Same as setup_webso.py but:
  - Always does a full DB reset before starting
  - Migrates only the first MINI_LIMIT records of each entity

Usage:
    python setup_webso_mini.py
"""
...
MINI_LIMIT       = 10   # max records per entity type

# ─────────────────────────────────────────────────────────────────────────────
# DATABASE RESET (always runs)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# PIPEDRIVE FETCH (single page, max MINI_LIMIT records)
# ─────────────────────────────────────────────────────────────────────────────

def fetch_mini(endpoint: str) -> list:
    url = f"https://api.pipedrive.com/{endpoint}"
    resp = requests.get(url, params={"api_token": PIPEDRIVE_TOKEN, "start": 0, "limit": MINI_LIMIT}, timeout=30)
    if resp.status_code >= 400:
        return []
    data = resp.json()
    if not data.get("success"):
        return []
    return (data.get("data") or [])[:MINI_LIMIT]
...
def fetch_stages() -> dict[int, str]:
    """Return {stage_id: twenty_stage} using Pipedrive stage names."""
    STAGE_NAME_MAP = {
        # English
    ...
    for s in resp.json().get("data") or []:
        ...
        twenty_stage = STAGE_NAME_MAP.get(name.lower(), "NEW")
        result[s["id"]] = twenty_stage
    return result
...
def html_to_markdown(html: str) -> str:
    """Convert Pipedrive HTML note content to plain Markdown."""
    if not html:
        return ""
    md = _h2t.handle(html)
    md = re.sub(r'\n{3,}', '\n\n', md)
...
def note_title(markdown: str) -> str:
    """Extract a clean title from the first non-empty line, cut at word boundary."""
...
def migrate_data(ws: str):
    console.print(f"\n[bold cyan]Step 4: Migrating Pipedrive data (max {MINI_LIMIT} each)...[/bold cyan]")

    console.print("  [dim]Fetching from Pipedrive...[/dim]")
    stage_map  = fetch_stages()
    orgs       = fetch_mini("v1/organizations")
    persons    = fetch_mini("v1/persons")
    deals      = fetch_mini("v1/deals")
    leads      = fetch_mini("v1/leads")
    # Notes: global paginated fetch, then slice to MINI_LIMIT
    notes = fetch_all_pages("v1/notes")[:MINI_LIMIT]

    # Activities: fetch per deal (global endpoint returns 0 due to Pipedrive limitation)
    seen_activity_ids: set[int] = set()
    activities: list = []
    for d in deals:
        for a in fetch_all_pages(f"v1/deals/{d['id']}/activities"):
            if a["id"] not in seen_activity_ids:
                seen_activity_ids.add(a["id"])
                activities.append(a)
    for org in orgs:
        for a in fetch_all_pages(f"v1/organizations/{org['id']}/activities"):
            if a["id"] not in seen_activity_ids:
                seen_activity_ids.add(a["id"])
                activities.append(a)

    console.print(
        f"  [green]Fetched: {len(orgs)} orgs, {len(persons)} persons, "
        f"{len(deals)} deals, {len(leads)} leads, {len(notes)} notes, "
        f"{len(activities)} activities[/green]"
    )
    ...
    with psycopg.connect(NEON_DB_URL) as conn:
        ...
        org_to_opps: dict[int, list[str]] = {}  # pipedrive org_id → [twenty opp ids]
        for d in deals:
            oid = str(uuid.uuid4())
            rows = insert(cur, f"""
        ...
        for a in activities:
            ...
            # If activity has no deal link, also attach it to all opportunities for this org
...
def main():
    console.print(Panel(
        f"[bold]Webso CRM Mini Setup[/bold]\n"
        f"pekka@webso.fi · Pipedrive → Twenty\n"
        f"[yellow]Max {MINI_LIMIT} records per entity · Full DB reset[/yellow]",
        title="Mini Setup"
    ))
    ...
    console.print(Panel(
        "[bold green]Mini setup complete![/bold green]\n\n"
